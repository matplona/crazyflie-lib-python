...
class Logger:
    ...
    def add_variable(self, group, name, period_in_ms, type) -> None:
        ...
        """
        variable_log = None
        if("{}.{}".format(group,name) in self._variables):
    ...
    def start_logging_variable(self, group, name):
        ...
        log : LogConfig = self._variables["{}.{}".format(group,name)]["log"]
    ...
    def start_logging_group(self, group):
        ...
        for name, var in self._variables.items():
            if(name.startswith(group) and var["log"]["status"]== STOPPED):
                var["log"]["log"].start()
                var["log"]["status"] = STARTED
    def stop_logging_group(self, group):
        """Stop logging all the variable in the group specified"""
        for name, var in self._variables.items():
            if(name.startswith(group) and var["log"]["status"]== STARTED):
                var["log"]["log"].stop()
                var["log"]["status"] = STOPPED
    ...
    def add_watcher(self, group, name, cb):
        """
        ...
        variable = self._variables["{}.{}".format(group,name)]
        variable["cb"] = cb
    def add_predicate(self, group, name, pred):
        """
...
class Setter:
    ...
    def _cb(self, name, value):
        ...
        value = float(value)
        #check if the predicate is true
        if(self._variables[name]["predicate"](value)):
            #call the callback with the following parameter:
                #   -   timestamp
                #   -   group.name
                #   -   value
            self._variables[name]["cb"](ts, name, value)
    ...
    def add_variable(self, group, name):
        ...
        self._cf.param.add_update_callback(group, name, self._cb)
    ...
    def add_watcher(self, group, name, cb):
        ...
        if(cb.__code__.co_argcount != 3):
            raise Exception("Watcher must accept exacty 3 parameter.")
        if("{}.{}".format(group, name) in self._variables):
            self._variables["{}.{}".format(group, name)]["cb"] = cb
        else:
    ...
    def add_predicate(self, group, name, pred):
        ...
        if(pred.__code__.co_argcount != 1):
            raise Exception("Predicate must accept exacty 1 parameter.")
        if("{}.{}".format(group, name) in self._variables):
            self._variables["{}.{}".format(group, name)]["predicate"] = pred
        else:
            raise Exception("Variable not found in the setter, you may add it before use.")
    ...
    def set_value(self, group, name, value, template="{:f}"):
        ...
        """
        if("{}.{}".format(group, name) in self._variables):
            self._cf.param.set_value("{}.{}".format(group, name), template.format(value))
        else:
            raise Exception("Variable not found in the setter, you may add it before use.")
    ...
    def get_value(self, group, name):
        ...
        """
        if("{}.{}".format(group, name) in self._variables):
            return self._cf.param.get_value("{}.{}".format(group, name))
        else:
            raise Exception("Variable not found in the setter, you may add it before use.")