...
def __record_angles_average(cf: Crazyflie) -> LhCfPoseSample:
...
def __record_angles_sequence(cf: Crazyflie, recording_time_s: float) -> list[LhCfPoseSample]:
    ...
    reader = LighthouseSweepAngleReader(cf, ready_cb)
    reader.start()
...
def __parse_recording_time(recording_time: str, default: int) -> int:
...
def __print_base_stations_poses(base_stations: dict[int, Pose]):
...
def __visualize(cf_poses: list[Pose], bs_poses: list[Pose]):
...
def __estimate_geometry(origin: LhCfPoseSample,
    ...
    __print_base_stations_poses(initial_guess.bs_poses)
    __visualize(initial_guess.cf_poses, initial_guess.bs_poses.values())
    ...
    __print_base_stations_poses(solution.bs_poses)
    ...
    print('  Base stations at:')
    __print_base_stations_poses(bs_scaled_poses)

    __visualize(cf_scaled_poses, bs_scaled_poses.values())

    return bs_scaled_poses


def __upload_geometry(cf: Crazyflie, bs_poses: dict[int, Pose]):
...
def connect_and_estimate(cf: Crazyflie):
    """Connect to a Crazyflie, collect data and estimate the geometry of the system"""
    print(f'Step 1. Connecting to the Crazyflie on uri {cf.link_uri}...')
    print('  Connected')
    print('')
    ...
    while do_repeat:
        ...
        print('  Recording...')
        measurement = __record_angles_average(cf)
        do_repeat = False
    ...
    recording_time_s = __parse_recording_time(recording_time, default_time)
    print('  Recording started...')
    samples = __record_angles_sequence(cf, recording_time_s)
    print('  Recording ended')

    print('Step 6. Estimating geometry...')
    bs_poses = __estimate_geometry(origin, x_axis, xy_plane, samples)
    print('  Geometry estimated')

    print('Step 7. Upload geometry to the Crazyflie')
    input('Press enter to upload geometry. ')
    __upload_geometry(cf, bs_poses)
